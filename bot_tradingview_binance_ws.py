...
# ✅ FIX: extração de preço robusta (aggTrade em vários formatos)
...
def extract_price(message):
    """
    Aceita:
      - dict simples com 'p' (aggTrade), 'c' (close), 'price'
      - dicts aninhados: {'data': {...}}, {'payload': {...}}, {'k': {...}}
      - lista/tupla na raiz (pega o primeiro item)
      - string JSON
    """
    try:
        # Se vier string => parse
        if isinstance(message, str):
            message = json.loads(message)

        # Se vier lista/tupla, usa o primeiro item
        ...
        def on_msg(_, message):
            ...
            try:
                ...
                if px > 0:
                    ...
                    last_tick_ts = time.time()
                    print(f"=> Preco do px else: ",px)
                    pass
            except Exception as e:
                print(f"{YELLOW}⚠️ xffa1_on_msg error:{RESET}", e)
                traceback.print_exc()
        ...
        def heartbeat():
            while True:
                ...
                try:
                    ...
                    # print(f"{MAGENTA}💓 Heartbeat mod enviado (ping {HEARTBEAT_S}s) | Uptime: {uptime}s{RESET}")
                except Exception:
                    break

        start_ts = time.time()
        try:
            # algumas versões do SDK usam 'id', outras 'stream_id'
            try:
